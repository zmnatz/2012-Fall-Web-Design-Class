...
public class OrderForm extends Applet implements ActionListener,ItemListener {
	...
	public void actionPerformed(ActionEvent arg0) {
		...
		order.setItemId(itemIds.getSelectedIndex()==0 ? "" : itemIds.getSelectedItem());
		try{
			order.setQuantity(Integer.parseInt(quantity.getText()));
		}catch(NumberFormatException e){ }
		order.setCreditCard(creditCards.getSelectedCheckbox()==null ? 
				"" : creditCards.getSelectedCheckbox().getLabel());
		
		String errMsg = orderFormController.submit(order);
		if(errMsg!=null)
			alertError(errMsg);
		else{
			clearForm();
